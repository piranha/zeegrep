...
pub fn printChangedLines(writer: anytype, color: bool, path: []const u8, old: []const u8, new: []const u8) !usize {
    var n: usize = 0;
    ...
    while (true) : (line_no += 1) {
        ...
        try writer.print("── {f}:{f} ──\n", .{ ansi.styled(color, .path, path), ansi.styled(color, .heading, line_no) });
        try writer.print("- {f}\n", .{ansi.styled(color, .del, ol)});
        try writer.print("+ {f}\n", .{ansi.styled(color, .add, nl)});
    }
    return n;
}
